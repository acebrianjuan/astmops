...
TargetReportExtractor::TargetReportExtractor()
{
    // Initialize counters.
...
void TargetReportExtractor::addData(const Asterix::Record &rec)
{
    ...
    last_times_[rt.sys_typ_] = rec.timestamp_;
    ++counters_[rt.sys_typ_].in;
    if (isRecordToBeKept(rt, rec))
    {
        TargetReport tgtRep = makeTargetReport(rec);
        tgt_reports_[rt.sys_typ_].enqueue(tgtRep);

        ++counters_[rt.sys_typ_].out;
    }
}
...
bool TargetReportExtractor::isRecordToBeKept(RecordType rt, const Asterix::Record &rec) const
{
    if (rt.isUnknown())
    {
        return false;
...
TargetReport TargetReportExtractor::makeTargetReport(const Asterix::Record &rec) const
{
    Q_UNUSED(rec);
    return TargetReport();
}
